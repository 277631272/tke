...
type ControlPlaneComponent struct {
	// +optional
	ExtraArgs map[string]string `json:"extraArgs,omitempty" protobuf:"bytes,1,name=extraArgs"`
}

// APIServer holds settings necessary for API server deployments in the cluster
type APIServer struct {
	ControlPlaneComponent `protobuf:"bytes,1,opt,name=controlPlaneComponent"`
}
...
type DockerInfo struct {
	// +optional
	ExtraArgs      map[string]string `json:"extraArgs,omitempty" protobuf:"bytes,1,name=extraArgs"`
	Os             string            `json:"os,omitempty" protobuf:"bytes,2,name=os"`
	Runtime        string            `json:"runtime,omitempty" protobuf:"bytes,3,name=runtime"`
	RuntimeVersion string            `json:"runtimeVersion,omitempty" protobuf:"bytes,4,name=runtimeVersion"`
}
...
type Networking struct {
	...
	// +optional
	NetworkDevice string `json:"networkDevice,omitempty" protobuf:"bytes,2,opt,name=networkDevice"`
	// +optional
	ClusterCIDR string `json:"clusterCIDR,omitempty" protobuf:"bytes,3,opt,name=clusterCIDR"`
	// ServiceCIDR is used to set a separated CIDR for k8s service, it's exclusive with MaxClusterServiceNum.
	// +optional
	ServiceCIDR string `json:"serviceCIDR,omitempty" protobuf:"bytes,4,opt,name=serviceCIDR"`
	// +optional
	EniSubnetIds []string `json:"eniSubnetIds,omitempty" protobuf:"bytes,5,opt,name=eniSubnetIds"`
	// +optional
	EniRecyclePeriod *int64 `json:"eniRecyclePeriod,omitempty" protobuf:"bytes,6,opt,name=eniRecyclePeriod"`
	// DNSDomain is the dns domain used by k8s services. Defaults to "cluster.local".
	// +optional
	DNSDomain string `json:"dnsDomain,omitempty" protobuf:"bytes,7,opt,name=dnsDomain"`
	// +optional
	NetworkArgs map[string]string `json:"networkArgs,omitempty" protobuf:"bytes,8,name=networkArgs"`
	// +optional
	KubeProxyMode string `json:"kubeProxyMode,omitempty" protobuf:"bytes,9,name=kubeProxyMode"`
}
...
type ClusterFeature struct {
	...
	// +optional
	FeatureGates map[string]bool `json:"featureGates,omitempty" protobuf:"bytes,23,opt,name=featureGates"`
}
...
type ClusterProperty struct {
	...
	// +optional
	Propertys map[string]string `json:"propertys,omitempty" protobuf:"bytes,4,opt,name=propertys"`
}
