...
type MachineLister interface {
	...
	List(selector labels.Selector) (ret []*v2.Machine, err error)
	// Get retrieves the Machine from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*v2.Machine, error)
...
// List lists all Machines in the indexer.
func (s *machineLister) List(selector labels.Selector) (ret []*v2.Machine, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v2.Machine))
	})
	return ret, err
}

// Get retrieves the Machine from the index for a given name.
func (s *machineLister) Get(name string) (*v2.Machine, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v2.Resource("machine"), name)
	}
	return obj.(*v2.Machine), nil
}
