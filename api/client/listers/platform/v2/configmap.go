...
type ConfigMapLister interface {
	...
	List(selector labels.Selector) (ret []*v2.ConfigMap, err error)
	// Get retrieves the ConfigMap from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*v2.ConfigMap, error)
...
// List lists all ConfigMaps in the indexer.
func (s *configMapLister) List(selector labels.Selector) (ret []*v2.ConfigMap, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v2.ConfigMap))
	})
	return ret, err
}

// Get retrieves the ConfigMap from the index for a given name.
func (s *configMapLister) Get(name string) (*v2.ConfigMap, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v2.Resource("configmap"), name)
	}
	return obj.(*v2.ConfigMap), nil
}
