...
type ClusterCredentialLister interface {
	...
	List(selector labels.Selector) (ret []*v2.ClusterCredential, err error)
	// Get retrieves the ClusterCredential from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*v2.ClusterCredential, error)
...
// List lists all ClusterCredentials in the indexer.
func (s *clusterCredentialLister) List(selector labels.Selector) (ret []*v2.ClusterCredential, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v2.ClusterCredential))
	})
	return ret, err
}

// Get retrieves the ClusterCredential from the index for a given name.
func (s *clusterCredentialLister) Get(name string) (*v2.ClusterCredential, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v2.Resource("clustercredential"), name)
	}
	return obj.(*v2.ClusterCredential), nil
}
