...
type ClusterAddonLister interface {
	...
	List(selector labels.Selector) (ret []*v2.ClusterAddon, err error)
	// Get retrieves the ClusterAddon from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*v2.ClusterAddon, error)
...
// List lists all ClusterAddons in the indexer.
func (s *clusterAddonLister) List(selector labels.Selector) (ret []*v2.ClusterAddon, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v2.ClusterAddon))
	})
	return ret, err
}

// Get retrieves the ClusterAddon from the index for a given name.
func (s *clusterAddonLister) Get(name string) (*v2.ClusterAddon, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v2.Resource("clusteraddon"), name)
	}
	return obj.(*v2.ClusterAddon), nil
}
