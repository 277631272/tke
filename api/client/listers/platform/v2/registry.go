...
type RegistryLister interface {
	...
	List(selector labels.Selector) (ret []*v2.Registry, err error)
	// Get retrieves the Registry from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*v2.Registry, error)
...
// List lists all Registries in the indexer.
func (s *registryLister) List(selector labels.Selector) (ret []*v2.Registry, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v2.Registry))
	})
	return ret, err
}

// Get retrieves the Registry from the index for a given name.
func (s *registryLister) Get(name string) (*v2.Registry, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v2.Resource("registry"), name)
	}
	return obj.(*v2.Registry), nil
}
