...
type CronHPALister interface {
	...
	List(selector labels.Selector) (ret []*v2.CronHPA, err error)
	// Get retrieves the CronHPA from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*v2.CronHPA, error)
...
// List lists all CronHPAs in the indexer.
func (s *cronHPALister) List(selector labels.Selector) (ret []*v2.CronHPA, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v2.CronHPA))
	})
	return ret, err
}

// Get retrieves the CronHPA from the index for a given name.
func (s *cronHPALister) Get(name string) (*v2.CronHPA, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v2.Resource("cronhpa"), name)
	}
	return obj.(*v2.CronHPA), nil
}
